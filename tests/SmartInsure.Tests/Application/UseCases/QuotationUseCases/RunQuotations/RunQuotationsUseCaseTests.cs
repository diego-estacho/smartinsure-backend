...
/// <summary>RN-056/RN-057/RN-060 — Solicitação de Cotações (fan-out request side): escopo, materialização e re-solicitação. RN-103 — Corretora vem do Escopo ativo.</summary>
[Trait("RuleId", "RN-056")]
[Trait("RuleId", "RN-057")]
[Trait("RuleId", "RN-103")]
public class RunQuotationsUseCaseTests
{
    ...
    public RunQuotationsUseCaseTests()
        => _useCase = new RunQuotationsUseCase(
            _groupRepository, _enablementRepository, _quotationRepository, _channel, _currentUser, _unitOfWork);
    ...
    public async Task Execute_DeveMaterializarEEnfileirarTodasHabilitadas_NoModoAll()
    {
        ...
        var response = await _useCase.ExecuteAsync(new RunQuotationsRequest(group.Id), CancellationToken.None);
    ...
    [Fact]
    public async Task Execute_DevePersistirCorretoraDoEscopoAtivoNoGrupo_ParaOReconciliador()
    {
        var brokerageId = Guid.CreateVersion7();
        _currentUser.ActiveBrokerageId.Returns(brokerageId);
        var group = GroupAll();
        ...
        await _useCase.ExecuteAsync(new RunQuotationsRequest(group.Id), CancellationToken.None);

        // RN-103: a Corretora persistida é a do Escopo ativo, não um valor vindo do cliente.
    ...
    {
        // RN-103: sem Corretora ativa no acesso (ex.: Usuário só com vínculo de Tomador), a solicitação é recusada.
        _currentUser.ActiveBrokerageId.Returns((Guid?)null);
        var group = GroupAll();
        _groupRepository.GetByIdWithInsurersAsync(group.Id, Arg.Any<CancellationToken>()).Returns(group);

        var act = () => _useCase.ExecuteAsync(new RunQuotationsRequest(group.Id), CancellationToken.None);

        await act.Should().ThrowAsync<BusinessRuleException>();
        await _enablementRepository.DidNotReceive().ListActiveByBrokerageAsync(Arg.Any<Guid>(), Arg.Any<CancellationToken>());
    }
    ...
    {
        _currentUser.ActiveBrokerageId.Returns(Guid.CreateVersion7());
        ...
        var act = () => _useCase.ExecuteAsync(
            new RunQuotationsRequest(Guid.CreateVersion7()), CancellationToken.None);
