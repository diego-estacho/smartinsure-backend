...
/// <summary>
/// Modalidade Importada (RN-033): a modalidade como a Seguradora a oferece, trazida na importação
/// exatamente como exposta. Reencontrada pelo identificador de origem (SourceId), por Seguradora.
/// Nome, ramo, público-alvo e parâmetros comerciais vêm da fonte e não são editados à mão (RN-033).
/// </summary>
public sealed class ImportedModality : EntityBase
{
    ...
    /// <summary>Identificador da Modalidade Global (GlobalModalities[].Id no PlugV2) — resolve o vínculo (RN-035).</summary>
    ...
    /// <summary>RN-035/RN-037: Modalidade (lado Smart) a que esta Importada está vinculada. Nulo = exceção sem vínculo (vai à Fila).</summary>
    public Guid? ModalityId { get; private set; }

    /// <summary>RN-035/RN-037: origem do vínculo — Automatic (id global) ou Manual (override do Administrador).</summary>
    public EModalityLinkSource? ModalityLinkSource { get; private set; }

    /// <summary>Parâmetros comerciais preservados como recebidos da fonte (RN-033).</summary>
    public string? CommercialParameters { get; private set; }

    public EImportedModalityStatus Status { get; private set; }

    /// <summary>RN-037: marcada como Ignorada na Fila — não é oferecida e não volta à fila. A importação não altera este marcador.</summary>
    ...
    /// <summary>
    /// RN-033: atualiza com os dados atuais da fonte, preservando identidade e mapeamento.
    /// RN-038: presença numa importação bem-sucedida reativa a Modalidade Importada.
    /// </summary>
    ...
    /// <summary>
    /// RN-035/RN-037: vincula à Modalidade. A importação (Automatic) resolve pelo id da Modalidade
    ...
    /// <summary>RN-038: ausência numa importação bem-sucedida da Seguradora desativa. Idempotente (automação).</summary>
    public void Deactivate() => Status = EImportedModalityStatus.Inactive;

    /// <summary>RN-037: o Administrador marca a Modalidade Importada como Ignorada na Fila.</summary>
    public void Ignore() => IsIgnored = true;

    /// <summary>RN-037: reavaliar um item antes ignorado — volta a poder entrar na Fila.</summary>
