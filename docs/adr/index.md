...
| [ADR-063](063-filial-como-pessoa-vinculada-a-matriz.md) | Filial como Pessoa vinculada à matriz, e estabelecimento cotado no Grupo de Cotação | accepted | dominio, integracoes | Filial é `Person` importada pelo mesmo fluxo do Birô, com vínculo persistido `HeadquartersPersonId` (não derivado por raiz de CNPJ); Filial sem Papel da Pessoa, então RN-016 segue íntegra; `QuotationGroup.BranchPersonId` anulável = estabelecimento cotado (ausente = matriz); falha do Birô na Filial preserva a matriz |
