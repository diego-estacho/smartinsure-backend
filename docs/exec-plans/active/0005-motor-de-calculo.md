...
- [x] Migration no `smartinsure-dbmigration`: `V20260719060014__criar-tabela-insurer-enablements.sql` (FK `BrokerageId → Persons`, `InsurerId → Insurers`; unique no par; Status string; parâmetros de conexão).
- [x] Core: `InsurerEnablement` (entidade rica), `EInsurerEnablementStatus`, `ECalculationEngine` (PlugV2), `IInsurerEnablementRepository`, `ICalculationEngine` + `ICalculationEngineResolver`.
- [x] Integration: `CalculationEngines/` (options PlugV2, `PlugV2CalculationEngine`, `CalculationEngineResolver` com motores por keyed DI); client Refit do PlugV2 entra com a primeira operação (OPEN-07).
...
- [x] Api: `InsurerEnablementsEndpoint` (autenticado, sem restrição de Perfil nesta fase — OPEN-07/OPEN-03).
...
- Migration: Flyway local aplicou `criar-tabela-insurer-enablements` com sucesso (agora em v20260719060014).
- Contrato: `docs/generated/openapi.json` com `/api/v1/insurer-enablements` (POST/GET), `/{id}` (PUT/GET) e `/{id}/status` (PATCH).
