...
- **Fatia B — CPF (backend, 2026-08-07):** RN-082 (CPF obrigatório/imutável/único nos fluxos §8 RN-068/069/070; RN-066 corretor-admin sem CPF) + revisão da RN-001. `CpfValidator` (Infra.CrossCutting); `User.DocumentNumber` (nullable, `Create` normaliza/valida 11 dígitos); `UserMapping` (índice único filtrado); migration `V20260807140000__adicionar-cpf-em-users.sql` (dbmigration, coluna nullable + índice filtrado via EXEC). CPF propagado nos 3 fluxos escopados (request/validator/usecase/endpoint body) via `InviteUserCommand`; exposto no detalhe (`GetUserResponse`); busca cobre CPF. `dotnet build` 0 erros; `dotnet test` = **783/783**.
  - **Pendente (acoplado):** regen do contrato + front do CPF (modal §8 com `SiDocField`, detalhe, composable enviando `documentNumber`) entram JUNTOS — os bodies de convite agora exigem `documentNumber`, então regenerar sem o front quebraria o typecheck. Backend B ainda **não commitado** (evita drift do contrato).
- Próximo: front da Fatia B (CPF) + regen; depois C (editar), D (reset senha), E (último acesso), F (criação inline — consome a sessão de Perfis).
