# Exec-plan 0011 — Hierarquia de criação: Administrador convida Corretor Administrador (RN-066, fatia 3a)

Status: em andamento — fatia 3a da jornada Perfis e Permissões (slug `perfis-permissoes`, AB# pendente). Sobre as fatias 0/1/2.
Contexto obrigatório: `AGENTS.md`, `docs/BACKEND.md`, `docs/SECURITY.md`, RN-066/RN-067/RN-068 (`regras-de-negocio/perfis-e-permissoes.md`), RN-012 (`usuarios.md`), glossário (`BrokerageAdministrator`/`PolicyHolderAdministrator`), ADR-013/ADR-014 (autorização por policy/claims), [OPEN-19](../../product-specs/open-decisions.md).

## Objetivo

Primeiro degrau da hierarquia: o Administrador do Sistema convida um Corretor Administrador informando as Corretoras (RN-066). Semeia os Perfis fixos globais Corretor Administrador e Tomador Administrador (RN-062/RN-073). Reusa o Convite de primeiro acesso (RN-065, fatia 2).
...
- **Fora do escopo (diferido, com motivo):**
  - **RN-067** (ampliar Corretoras de um Usuário): o Perfil atribuído na ampliação "segue as regras de criação (RN-068 a RN-071)" — subespecificado pela RN-067 isolada; acopla às regras de criação de Perfil por escopo (fatia 4, parte sob OPEN-17). Implementar sem isso seria inventar qual Perfil atribuir. Fica para depois da fatia 4.
  - **RN-068** (Corretor Administrador cria Tomador Administrador): exige a **Corretora ativa** (fatia 1b), travada pela [OPEN-19](../../product-specs/open-decisions.md) (ADR-065 proposto + padrão de primeiro acesso PO).
...
- Usuário nasce Pendente com Convite (RN-065) e um vínculo Corretor Administrador por Corretora informada.
- Perfis fixos CA/TA resolvidos por chave natural (IsFixed+Scope+Name), nunca por GUID.

## Evidências

- Backend: `rtk dotnet test` **322/322** (novos `[Trait("RuleId","RN-066")]`: convida CA com um vínculo por corretora; recusa corretora inativa/inexistente; recusa e-mail duplicado sem criar identidade). `check-harness.py` → `harness ok`.
...
- Pendências: aplicar migration; RN-067 (após fatia 4) e RN-068 (após fatia 1b/OPEN-19); AB#/PBI; PR.
