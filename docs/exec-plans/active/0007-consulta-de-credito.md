...
- Adequação ao shape real do PlugV2 (2026-07-21, validado com consulta real em QA — CNPJ de teste): grupos dinâmicos por ModalityGroupName, máximo do grupo, utilizado derivado (revisado − disponível), razão social do tomador, correção de 404 na URL do gateway (barra final), migration reescrita pré-merge (tabela CreditInquiryResultLimits)
- Review incremental em dois eixos (2026-07-21): backend — ThenInclude de Limits (coleção vinha vazia no detalhe), drift GroupType/Rate migration×mapping, campo morto por-resultado removido do contrato, teste de agregação multi-modalidade (295/295); front — E2E reforçados (utilizado > 0, união de colunas com "—", barra com percentual, validade "—"), denominador do percentual corrigido (utilizado/revisado). Descartado com justificativa: percentual de barra no cliente (apresentação, valores já servidos)
