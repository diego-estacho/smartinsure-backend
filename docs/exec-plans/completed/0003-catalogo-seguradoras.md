...
- [x] RN-005..RN-010 catalogadas e aprovadas; glossário com termos/status propostos (ratificação da PO pendente — registrada no próprio glossário).
...
- [x] Application: `CreateInsurer` (RN-005, com `CnpjValidator` de dígitos verificadores em Infra.CrossCutting), `UpdateInsurer` (RN-006), `ChangeInsurerStatus` (RN-007), `ListInsurers`/`GetInsurer` (RN-008), `SetUserProfile` (RN-010, com proteção de último admin e invalidação de cache de perfil). Situação/perfil desconhecidos viram `BusinessRuleException` (defense-in-depth além do validator).
- [x] Api: `InsurersEndpoint` (escrita com policy `SystemAdministrator`; leitura autenticada), rota `PUT /users/{id}/profile`, `UserProfileClaimsTransformation` (perfil → role, cache 5 min invalidado na concessão/revogação), policy fail-closed registrada (RN-009).
...
- Testes: `dotnet test tests/SmartInsure.Tests` — **119/119 aprovados** (59 da baseline + 60 novos, todos com `[Trait("RuleId", ...)]` RN-005..RN-010; NetArchTest incluso; TDD com evidência RED→GREEN por task).
...
- Smoke e2e (API real + JWT local + SQL local, 2026-07-16) — **8/8**:
  ...
  - S2 (RN-009): `POST /insurers` autenticado sem perfil → 403, sem efeito.
  - S3 (RN-009): `POST /insurers` como Administrador do Sistema (runbook simulado via SQL) → 201.
  - S4 (RN-005): mesmo CNPJ de novo → 409.
  - S5 (RN-007): `PATCH /insurers/{id}/status` → 200; repetido na mesma situação → 409.
  - S6/S7 (RN-008): Inativa ausente para não-admin (mesmo com `includeInactive=true`); visível para admin com status por nome estável `Inactive`.
