...
- [x] Parametrização por vínculo: `ConnectionParameters` do PlugV2 = `{"baseUrl","key"}` validado na gravação (`PlugV2ConnectionParameters.Parse`); CNPJ da Corretora vem do vínculo; `Insurer.ReferenceExternalId` (opcional, string livre até 100 — cada motor usa o formato do seu sistema de origem; migration `V20260720055100`) guarda o id da Seguradora no sistema de origem. Sem configuração global de motor. Pendência: avaliar cifragem da `key` (credencial em coluna).
