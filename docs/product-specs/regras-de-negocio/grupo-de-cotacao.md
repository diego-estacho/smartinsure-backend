...
**Descrição.** Ao concluir a etapa de dados de risco do wizard de nova oferta, a plataforma persiste um Grupo de Cotação em Rascunho, reunindo o que o corretor montou até ali: o Tomador, o estabelecimento cotado (RN-053), o Segurado, o escopo de Seguradoras a cotar, a Modalidade, o valor segurado, a vigência e as Coberturas Adicionais marcadas. Existe um único Grupo de Cotação por jornada; enquanto Rascunho, revisões das etapas anteriores atualizam o mesmo grupo (RN-051), nunca criam um novo.
...
## RN-053 — Estabelecimento cotado no Grupo de Cotação

**Descrição.** O Grupo de Cotação registra qual estabelecimento do Tomador é o objeto da garantia: a matriz ou uma das suas Filiais (RN-052). O Tomador do grupo continua sendo sempre a matriz (RN-016); a Filial é uma escolha do corretor dentro dela, no máximo uma por Grupo de Cotação.

**Pré-condições.** Jornada de nova oferta com Tomador selecionado. Filiais do Tomador cadastradas (RN-052), quando houver escolha a fazer.
...
**Casos limite.** Filial inexistente, ou vinculada a outra matriz que não o Tomador do grupo: recusada — a validação de vínculo é do servidor. Troca do Tomador do Grupo de Cotação: o estabelecimento cotado é limpo e volta a ser a matriz. Grupo de Cotação criado antes desta regra permanece válido, com a matriz como estabelecimento cotado. **Limite de Crédito e taxa são sempre da matriz**, marcada ou não uma Filial: a Seguradora não consulta limite pelo CNPJ da Filial, porque o risco é calculado sobre a matriz mesmo quando a emissão é para uma Filial — a escolha do estabelecimento não altera os limites apresentados (RN-029, [OPEN-17](../open-decisions.md)). Qual CNPJ é efetivamente enviado à Seguradora ao cotar depende da etapa de cotações, que segue mockada e em aberto ([OPEN-07](../open-decisions.md)).
