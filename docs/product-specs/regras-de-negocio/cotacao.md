...
> **Listagem de Cotações (Fatia 1 — catalogada em 2026-07-30):** o "livro" de Cotações da Corretora — **uma linha por Cotação** (não por Grupo de Cotação), read-only, paginado e filtrável no servidor (RN-077), com a **situação apresentada** derivada do resultado classificado (RN-078). Entra como demanda própria, separada do Passo 4. A **Página de Detalhes** (Fatia 1 — read-only) foi catalogada em 2026-08-04 (RN-081); o **cancelamento** das Cotações segue como fatia seguinte; **emissão** e **followup** seguem fora.
...
## RN-081 — Página de Detalhes da Cotação (Fatia 1 — read-only)

> Catalogada em 2026-08-04. Demanda seguinte à Listagem de Cotações (RN-077/RN-078). **Não cria status novo nem transição**: o detalhe é apresentação read-only, deriva do já persistido. **Emissão**, **cancelamento**, **Documentos** e **followup** seguem como fatias/demandas próprias, fora desta.

**Descrição.** A partir do livro de Cotações (RN-077), o corretor abre o **detalhe de uma Cotação** para conferir, numa só tela, os dados do pedido e o andamento. O detalhe apresenta: Tomador e Segurado (com nome e CNPJ), Seguradora, Modalidade, importância segurada, prêmio, comissão (percentual e valor), Coberturas Adicionais contempladas, vigência (início e fim), data de criação, o número da Cotação quando a Seguradora o informou, a **situação apresentada** (RN-078) e o **indicador de exigência de Contragarantia (CCG)** quando houver — dado **ortogonal** à situação (RN-058). Apresenta também uma **cronologia** com os marcos que a plataforma conhece do pedido (criação da Cotação, obtenção do resultado da Seguradora e, quando aplicável, a exigência de CCG). É leitura: nenhuma ação altera a Cotação nesta fase.

**Pré-condições.** Cotação obtida e presente no livro da Corretora do Escopo ativo (RN-077). Acesso autenticado com Corretora ativa no Escopo (RN-064).

**Critério de aceitação.** O detalhe é resolvido pela **identidade da Cotação** (não pelo número, que pode faltar). A plataforma só apresenta o detalhe de Cotações da **Corretora do Escopo ativo** (RN-064): pedido de detalhe de uma Cotação de outra Corretora é tratado como inexistente (não revela existência). A situação apresentada e o indicador de CCG saem pelas mesmas regras da listagem (RN-078, RN-058), calculados no servidor por nome estável. A comissão em valor e as demais quantias vêm do que foi persistido na Cotação — nunca recalculadas no cliente. A cronologia contém apenas marcos reais que a plataforma registra; não são inventados eventos que ela não conhece.

**Casos limite.** Cotação sem número informado pela Seguradora: o detalhe abre normalmente e o número aparece vazio. Cotação em situação *Indisponível* ou *Não reconhecida*: o detalhe abre em leitura, com a situação apresentada, sem oferecer emissão. A **validade da proposta** e a **descrição livre do objeto** não são apresentadas nesta fase (não modeladas — seguem como decisão própria). CCG é indicador ortogonal e **não** bloqueia a leitura nem a situação apresentada.
