...
**Descrição.** No cadastro de uma Corretora, a plataforma consulta os dados de um CNPJ para o usuário revisar antes de confirmar, sem gravar nada na base. A consulta reaproveita os dados já cadastrados quando a Pessoa jurídica existir, ou os obtém do Birô quando não existir, sempre em modo somente leitura.

**Pré-condições.** Usuário autenticado informa um CNPJ válido de Pessoa jurídica durante o cadastro de Corretora.

**Critério de aceitação.** A plataforma devolve os dados cadastrais do CNPJ (razão social, nome fantasia, Natureza Jurídica, setor e endereço principal) sem criar nem alterar Pessoa, papel ou endereço. Se o CNPJ já possuir Papel da Pessoa de corretor, a plataforma sinaliza que a Corretora já está cadastrada e identifica o cadastro existente, sem alterá-lo.
