...
## RN-013 — Busca de Pessoa Jurídica por nome ou documento

**Descrição.** A plataforma localiza Pessoas Jurídicas cadastradas a partir de um trecho do nome (razão social ou nome fantasia) ou de um CNPJ, devolvendo sempre uma lista. Pessoa já cadastrada é devolvida da própria base, sem consulta ao Birô e sem atualização de dados.
...
**Critério de aceitação.** Busca por trecho de nome devolve todas as Pessoas Jurídicas cujo nome ou nome fantasia contém o trecho informado. Busca por CNPJ já cadastrado devolve a Pessoa Jurídica correspondente, sem gerar consulta ao Birô. No contexto de tomador, o resultado contém apenas matrizes (RN-016).
...
**Descrição.** Toda Pessoa Jurídica é classificada como setor público ou setor privado a partir do código de Natureza Jurídica, conforme a tabela de referência oficial (CONCLA).

**Pré-condições.** Tabela de Naturezas Jurídicas carregada na plataforma como dado de referência.

**Critério de aceitação.** Pessoa Jurídica importada com código presente na tabela recebe a classificação (pública ou privada) daquela Natureza Jurídica, e a classificação acompanha a pessoa nas consultas.
...
**Descrição.** No contexto de tomador, apenas a matriz (ordem `/0001` do CNPJ) pode figurar como Pessoa Jurídica. Quando o solicitante informa o CNPJ de uma filial, a plataforma resolve a matriz correspondente e a devolve com a filial informada pré-selecionada.
