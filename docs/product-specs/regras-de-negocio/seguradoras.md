...
## RN-007 — Criação de Seguradora no catálogo
...
## RN-008 — Alteração de dados cadastrais da Seguradora
...
**Casos limite.** Alteração para CNPJ já usado por outra Seguradora: recusada. Remoção do nome fantasia ou do logotipo: aceita (são opcionais). Seguradora inexistente: recusada com indicação clara. A situação (Ativa/Inativa) não muda por alteração cadastral — somente pela ativação/desativação (RN-009).

## RN-009 — Ativação e desativação de Seguradora
...
## RN-010 — Consulta operacional exclui Seguradoras Inativas
...
## RN-011 — Escrita no catálogo exclusiva do Administrador do Sistema
