...
Parcialmente resolvida em 2026-07-27 (jornada Cotação): a **etapa de cotações** entrou em escopo — solicitar Cotações às Seguradoras a partir do Grupo (escopo *todas* × *escolhidas*, RN-052), obter e persistir cada Cotação por Seguradora com tolerância a falha isolada (RN-053), classificar o resultado de forma estável com esteira e motivos (RN-054), selecionar uma Cotação seguível (RN-055), recalcular/invalidar por qualquer mudança de dado com confirmação bloqueante (RN-056) e a validade da Cotação (RN-057). Direção do disparo confirmada: *todas as habilitadas* é o padrão recomendado, com *escolhidas* como alternativa; o disparo inclui Seguradoras que não ofertam a Modalidade (retornam indisponível com motivo) por decisão de transparência ao corretor. Seguem **fora de escopo** (demanda própria): o encaminhamento da Cotação escolhida (followup da análise de subscrição, aceite como Proposta e emissão), a Página de Listagem de Cotações, e quais Perfis podem solicitar cotação/gerenciar Habilitação (OPEN-03). A confirmar: o prazo de validade da Cotação (~15 dias) e sua fonte no contrato do fornecedor. Refinado com autonomia do time; **aguardando ratificação da PO** (a esta ratificação cabe fechar a entrada, junto com a emissão).
