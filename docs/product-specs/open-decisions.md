...
## OPEN-17 — Efeito da Filial fora do Grupo de Cotação
Dono: PO (gerente de projeto)
...
Parcialmente resolvida em 2026-07-28 (decisão do dono): **o Limite de Crédito e a taxa são sempre da matriz** — a Seguradora não consulta limite pelo CNPJ da Filial, porque o risco é calculado sobre a matriz mesmo quando a emissão é para uma Filial. Portanto o item (b) está encerrado: a Consulta de Crédito **não** passa a aceitar Filial, e a Filial marcada no Grupo de Cotação não altera limites nem taxas (RN-029, RN-053). O mesmo princípio aponta a matriz como base do cálculo no item (a), mas a forma concreta depende da etapa de cotações, ainda mockada (OPEN-07), e será confirmada na demanda que a desmockar.
Segue em aberto: (a) qual CNPJ o Motor de Cálculo recebe ao cotar um Grupo de Cotação com Filial marcada, e o que a Filial altera na emissão — depende da etapa de cotações (OPEN-07); (c) se a Nomeação passa a valer por estabelecimento — hoje a unicidade é por par Tomador×Seguradora; (d) como uma Filial é removida ou desvinculada da matriz (o cadastro nasce sem remoção, como o vínculo de papel da RN-017).
